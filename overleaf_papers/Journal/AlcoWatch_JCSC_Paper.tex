...
\begin{abstract}
This paper introduces the PhysioWatchBAC system, a wearable biosensing system for real-time blood alcohol concentration (BAC) measurement integrated with vehicle ignition control. The method combines PPG, EDA, and temperature readings obtained from a Wear OS smartwatch, using a PhysioWatchBAC neural network BiLSTM model with temporal attention for on-device computation. The system sends 20-byte BAC status messages to an Arduino-based vehicle controller via BLE communication. A safety-prioritized loss function with 5x false negative penalty and region-calibrated climatic conditions improve model accuracy. The model achieved MAE 0.0082~g/dL, false negative rate 0.7\%, end-to-end latency 580~ms, and a model size of 22~KB using the TensorFlow Lite model on hardware-in-the-loop simulation for evaluation on artificial physiological signals. The vehicle-side FSM provides fail-safe ignition lockout functionality with timeout ignition monitoring and continuous watch wear verification.
\end{abstract}
...
Driving under the influence of alcohol remains an ongoing global healthcare concern, accounting for approximately 28\% of traffic deaths in the United States, with similar statistics in the rest of the world. Although legislation has been put in place to set the limit of 0.08~g/dL, actual enforcement remains challenging due to the restricted area covered by typical alcohol breath tests at road checkpoints, where drivers also know ways to evade the test by timing or routing. This has created the need for passive, non-stop monitoring systems using biosensors together with vehicle control systems. There are technical limitations to the current methods used for alcohol detection: accuracy remains variable for multi-parameter sensing, whereas fixed parameters are not effective in different environmental conditions, with the current breathalyzer systems requiring human interaction for ignition interlocks.
...
Machine learning approaches for BAC estimation from physiological signals have shown promise in recent research \cite{wang2024, zhang2024}. Sensor fusion techniques combining multiple physiological parameters improve accuracy compared to single-sensor approaches. Recent work on attention-based LSTM networks demonstrates effective multimodal physiological signal fusion for continuous health monitoring \cite{zhang2024}. Edge AI implementations using TinyML have enabled sophisticated inference on resource-constrained wearable devices \cite{kumar2025}. However, challenges remain in developing algorithms that generalize across diverse populations and environmental conditions while maintaining computational efficiency for embedded deployment \cite{chen2025}.

\subsection{Software Security in Automotive Systems}
Vehicle and driver monitoring systems require robust software security to prevent unauthorized access and manipulation. Research on vehicular interface security emphasizes the importance of encryption, authentication, and tamper detection algorithms \cite{sensors2024, sensors2023}. Recent advances in secure BLE communication protocols for safety-critical automotive applications provide validated design patterns for reliable data exchange \cite{martinez2024}. The integration of biometric authentication with vehicular systems presents unique software challenges in balancing security with user experience.
...
PhysioWatchBAC is divided into three hardware components: the Wear OS smartwatch running the sensor reading and neural network inference, the Arduino Nano 33 BLE module running the ignition relay control, and the custom BLE GATT service handling the encrypted status transfer. Figure~\ref{fig:system} illustrates this distributed topology. From the computational perspective, the smartwatch performs the $O(n^2)$ attention and BiLSTM passes, and the Arduino performs the deterministic FSM with $O(1)$ transitions. This is achieved while keeping the power consumption below 35~mW and the decision time below 10~ms.
...
\bibitem{wang2024} Y. Wang, J. Chen, M. Zhang and L. Liu, Deep learning-based
physiological signal analysis for real-time health monitoring in wearable
devices, {\it IEEE Transactions on Biomedical Engineering} {\bf 71}(3)
(2024), pp. 892--904,
\url{https://doi.org/10.1109/TBME.2023.3321456}.

\bibitem{kumar2025} R. Kumar, S. Patel and A. Sharma, Edge AI for wearable
biosensors: A comprehensive review of TinyML implementations, {\it ACM
Computing Surveys} {\bf 57}(2) (2025), Article 42,
\url{https://doi.org/10.1145/3638529}.

\bibitem{zhang2024} H. Zhang, X. Li, W. Yang and Q. Zhao, Attention-based
LSTM networks for multimodal physiological signal fusion in continuous
health monitoring, {\it Expert Systems with Applications} {\bf 238}
(2024), Article 121892,
\url{https://doi.org/10.1016/j.eswa.2023.121892}.

\bibitem{martinez2024} A. Martinez, C. Rodriguez and J. Garcia, Secure
BLE communication protocols for safety-critical automotive applications:
Design and validation, {\it IEEE Transactions on Vehicular Technology}
{\bf 73}(8) (2024), pp. 11234--11248,
\url{https://doi.org/10.1109/TVT.2024.3387652}.

\bibitem{chen2025} L. Chen, Y. Wu, K. Tanaka and M. Nakamura, Real-time
alcohol impairment detection using smartwatch PPG signals and ensemble
machine learning, {\it Journal of Biomedical Informatics} {\bf 151}
(2025), Article 104612,
