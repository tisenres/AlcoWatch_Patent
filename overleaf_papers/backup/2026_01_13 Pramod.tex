...
Driving under the influence of alcohol remains an ongoing global healthcare concern, accounting for approximately 28\% of traffic deaths in the United States, with similar statistics in the rest of the world \citep{lombardo2020l}. Although legislation has been put in place to set the limit of 0.08~g/dL, actual enforcement remains challenging due to the restricted area covered by typical alcohol breath tests at road checkpoints, where drivers also know ways to evade the test by timing or routing \citep{das2023}. This has created the need for passive, non-stop monitoring systems using biosensors together with vehicle control systems \citep{watchover}. There are technical limitations to the current methods used for alcohol detection: accuracy remains variable for multi-parameter sensing, whereas fixed parameters are not effective in different environmental conditions, with the current breathalyzer systems requiring human interaction for ignition interlocks \citep{bush2024development}. \par
Patent literature documents various ignition interlock designs--U.S. Patents 5,736,965 and 7,113,834 specify breath-based lockout mechanisms \citep{uspat5736965, uspat7113834}, while Indian Patent 286703 adds GSM telemetry for fleet monitoring \citep{inpat286703}. All of these solutions have one big problem in common: breath sampling needs people to be aware of it and gives them a chance to evade detection. Transdermal alcohol monitoring provides passive detection; however, the processing of the diminished cutaneous ethanol signal require advanced algorithms \citep{fairbairn2021}. Recent advances in deep learning-based physiological signal analysis have enabled real-time health monitoring on wearable devices \citep{wang2024, zhang2024}. Verg\'{e}s et al.\ have designed a smartwatch-based BAC detection using hyperdimensional computing and shown promising results but the computational overhead limits its deployment in real-time scene \citep{verges2024}. Edge AI implementations using TinyML have enabled sophisticated inference on resource-constrained wearable devices \citep{kumar2025}. Multimodal sensors like heart rate variability, skin conductance, and temperature are fused together to improve estimation robustness over single-sensor methods, though security against replay attacks and spoofing requires attention \citep{sensors2024, sensors2023, martinez2024}. The design of a robust and generalized framework for BAC estimation under limited memory and latency of wearable microcontrollers remains a challenging task. Stone et al.\ \citep{alcowatch2025} have designed the AlcoWatch framework that uses smartwatch-based $\mu$EMA method to obtain ecological momentary density for the assessment and tracking of alcohol use, demonstrating that smartwatch platforms can be used in behavioural monitoring. Recent work on alcohol impairment detection using smartwatch PPG signals further validates the effectiveness of wearable sensing approaches \citep{chen2025}.
\par
\begin{figure*}[t]
\centering
...
\bmsection*{Use of AI}
No form of AI is used in the preparation of the manuscript.
