...
and produces a reproducible constellation of changes in peripheral
physiology. A comprehensive survey of these correlates is given by
\citet{sharma2012driver}, who review sensor technologies and
computational techniques for stress recognition across more than
two decades of literature. The physiological markers that are
simultaneously (i) strongly predictive of stress and (ii) accessible
from a wrist-worn consumer device are four in number.

The first and most widely studied marker is \textbf{heart-rate
variability} (HRV) derived from the photoplethysmography (PPG)
signal. Under stress, the parasympathetic branch of the autonomic
nervous system withdraws its tonic inhibition of the sinoatrial node,
and heart rate variability collapses. Time-domain indices such as
the root mean square of successive differences (RMSSD) and the
standard deviation of normal-to-normal intervals (SDNN) both decrease
substantially during acute stressors, and frequency-domain indices
show a characteristic shift toward the low-frequency band
(0.04--0.15~Hz) at the expense of the high-frequency band
(0.15--0.4~Hz). These changes can be observed reliably in
inter-beat intervals (IBIs) extracted from wrist PPG.

The second marker is \textbf{electrodermal activity} (EDA), also known
as galvanic skin response. EDA reflects eccrine sweat-gland activity,
which is controlled exclusively by the sympathetic branch of the
autonomic nervous system. During stress the tonic (slowly varying)
component of EDA rises, and the density and amplitude of phasic
peaks per minute increases. Because EDA is only weakly coupled to
physical workload, it is considered one of the ``cleanest'' peripheral
indicators of psychological stress, and it features prominently in
both \citet{schmidt2018wesad} and \citet{can2019stress}. Although
only selected consumer wearables currently ship a dedicated EDA
sensor, the WESAD protocol was specifically designed around wrist
EDA and therefore provides a high-quality labelled corpus for
supervised learning.

The third marker is \textbf{skin temperature}. Under sympathetic
activation, peripheral vasoconstriction redirects blood flow away
from the distal extremities; within a few minutes of a stressor,
wrist skin temperature typically drops by between 0.1 and 0.5~$^\circ$C
relative to baseline. Although the absolute magnitude of this change
is small compared with EDA, it is extremely cheap to measure, it
drifts slowly and therefore tolerates the low sampling rate
(4~Hz) that consumer wearables typically expose, and it is already
present on essentially every modern smartwatch.

The fourth marker is \textbf{three-axis acceleration}. Strictly
speaking, acceleration does not itself encode stress; it encodes
movement. However, movement is a crucial confound that must be
controlled for, because both PPG and EDA are contaminated by motion
artefacts. Incorporating the magnitude of the acceleration vector
as a feature allows the downstream model to learn to down-weight
signals that arrive simultaneously with high-magnitude motion. For
this reason we include accelerometer magnitude as one of the five
input features.

In combination, these four physiological channels cover both
branches of the autonomic nervous system (sympathetic via EDA
and temperature, parasympathetic-sensitive via HRV), one direct
cardiovascular channel (PPG), and one confound-control channel
(acceleration), and they are all accessible from a modern wrist
wearable. This combination informs the five-feature input vector
used throughout the rest of this report.

\section{Existing Stress Detection Datasets}
\label{sec:lit-datasets}

Supervised stress detection requires labelled multimodal physiological
...
this project are summarised in Table~\ref{tab:datasets}.
...
synchronised recordings from fifteen subjects (S2--S11 and S13--S17;
...
with the approach taken in this work, is summarised in
...
\textbf{BiLSTM + attention (this work)} & \textbf{WESAD} & \textbf{93.1\%}
& \textbf{Wrist-only, 70~KB TFLite, real-time on Wear~OS.} \\
\bottomrule
\end{tabular}
\end{table}
...
summarises its performance: 93.1\% accuracy on held-out WESAD data
at a deployment footprint of 70~KB, which is roughly four times
smaller than the unquantised baseline and hundreds of times smaller
...
On the research side, \citet{can2019stress} provide a comprehensive
survey of smart-phone and wearable stress-detection studies, cataloguing
more than forty published systems. The majority target office or
...
together LED colour, audio, and climate settings.
...
Our system therefore occupies an under-explored niche: a physiologically
driven, graduated, real-time cabin-environment controller that runs
on commodity hardware and that is decoupled from the vehicle's primary
...
\begin{enumerate}
  ...
  for drivers.} While \citet{schmidt2018wesad} provides the dataset,
  and \citet{can2019stress} surveys classification approaches, no
  published work combines a wrist-only feature set, a four-class
  ordinal output, and an on-device deployment under 80~KB.

  \item \textbf{No adaptive cabin response system.} Section~\ref{sec:lit-cabin}
  showed that all existing cabin personalisation systems are manual
  or emotion-recognition-based, not physiology-driven.
  ...
  magnitude larger than those of a Wear~OS device.

  \item \textbf{No integrated end-to-end open system.} Neither the
  commercial wearables reviewed in Section~\ref{sec:lit-wearables}
